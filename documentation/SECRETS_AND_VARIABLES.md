...
7. [Environment-Specific Secrets](#7-environment-specific-secrets)
...
| `POWERPLATFORM_ENVIRONMENT_URL` | ✅ Yes | Both workflows | Default Power Platform environment URL (fallback when not provided as workflow input) |

### Variables

> No repository-level **variables** (`vars.*`) are currently used by the workflows. All runtime configuration is passed via workflow inputs or secrets. If you add variables in the future, document them here.
...
| Sensitive | Yes — treat as a credential |
...
| Type | Secret |
| Format | UUID — `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` |
| Sensitive | Moderate — publicly discoverable but best kept secret |
| Workflows | `github-to-powerplatform.yml`, `powerplatform-to-github.yml` |
...
| Sensitive | Low — but kept as a secret for flexibility |
| Workflows | `github-to-powerplatform.yml`, `powerplatform-to-github.yml` |
...
## 4. Where Each Secret Is Used

| Workflow file | Secret | Purpose |
|---|---|---|
| `github-to-powerplatform.yml` | `POWERPLATFORM_APP_ID` | Authenticates to Power Platform to import the solution |
| `github-to-powerplatform.yml` | `POWERPLATFORM_CLIENT_SECRET` | Authenticates to Power Platform to import the solution |
| `github-to-powerplatform.yml` | `POWERPLATFORM_TENANT_ID` | Identifies the tenant for authentication |
| `github-to-powerplatform.yml` | `POWERPLATFORM_ENVIRONMENT_URL` | Fallback target environment when not provided as workflow input |
| `powerplatform-to-github.yml` | `POWERPLATFORM_APP_ID` | Authenticates to Power Platform to export the solution |
| `powerplatform-to-github.yml` | `POWERPLATFORM_CLIENT_SECRET` | Authenticates to Power Platform to export the solution |
| `powerplatform-to-github.yml` | `POWERPLATFORM_TENANT_ID` | Identifies the tenant for authentication |
| `powerplatform-to-github.yml` | `POWERPLATFORM_ENVIRONMENT_URL` | Fallback source environment when not provided as workflow input |
...
If you need to deploy to **multiple environments** (e.g. Development, Test, Production), use [GitHub Environments](https://docs.github.com/en/actions/deployment/targeting-different-environments/using-environments-for-deployment) to scope secrets per environment instead of storing them at the repository level.

### Setup

1. Go to **Settings** → **Environments** → **New environment**.
2. Create environments named e.g. `development`, `test`, `production`.
3. Add environment-scoped secrets under each environment (same four secret names).
4. Update the workflow YAML to reference the target environment:

```yaml
jobs:
  deploy-to-powerplatform:
    environment: production          # ← add this line
    runs-on: ubuntu-latest
```

This ensures the `production` secrets are used only when deploying to the production environment, and protects production deployments with optional required reviewers.
